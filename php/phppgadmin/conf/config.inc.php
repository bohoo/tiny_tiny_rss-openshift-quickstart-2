<?php
	...
	$conf['servers'][0]['desc'] = 'OpenShift';

	// Hostname or IP address for server.  Use '' for UNIX domain socket.
	// use 'localhost' for TCP/IP connection on this computer
	$conf['servers'][0]['host'] = getenv('OPENSHIFT_POSTGRESQL_DB_HOST');

	// Database port on server (5432 is the PostgreSQL default)
	$conf['servers'][0]['port'] = getenv('OPENSHIFT_POSTGRESQL_DB_PORT');
	...
	$conf['servers'][0]['defaultdb'] = getenv('OPENSHIFT_APP_NAME');

	// Specify the path to the database dump utilities for this server.
	// You can set these to '' if no dumper is available.
	$conf['servers'][0]['pg_dump_path'] = '/opt/rh/postgresql92/root/usr/bin/pg_dump';
	$conf['servers'][0]['pg_dumpall_path'] = '/opt/rh/postgresql92/root/usr/bin/pg_dumpall';
